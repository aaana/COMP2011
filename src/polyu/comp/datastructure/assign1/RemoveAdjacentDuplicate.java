...
//    }
    ...
    public static Stack<Character> removeAdjacentDuplicate(char[] array, int left,int right){
        if(left>right){
            return new Stack<>();
        }else{
            Stack<Character> temp = removeAdjacentDuplicate(array,left,right-1);
            if(temp.size()==0||temp.peek()!=array[right]) {
...
//        String a= "mississippi";
        ...
        Stack<Character> result = RemoveAdjacentDuplicate.removeAdjacentDuplicate(str.toCharArray(), 0, str.length() - 1);
