...
public class ImprovedSelectionSort {
    ...
    public static void improvedSelectionSort(int[] b){
        int n = b.length;
...
//        if (true) return;
        ...
        for (int j = 0; j < turns; j++) {
            for (int i = 0; i < size; i++)
                b[i] = a[i];
            improvedSelectionSort(b);
        }
        duration = (System.currentTimeMillis() - startTime);
        System.out.println(turns + " runs of improvedSelectionSort takes " + (duration / 1000.) + " seconds.");
        System.out.println(Arrays.stream(b).mapToObj(Integer::toString).collect(Collectors.joining(", ")));
